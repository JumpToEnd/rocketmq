...
public class DefaultMQPushConsumer extends ClientConfig implements MQPushConsumer {
     ...
     * 支持两种消息消费模式：
     * <p>
     * 集群模式：同一个消费者组（consumerGroup）内共享消息（只有一个消费者会消费到消息，配合负载均衡策略）、
     * 广播模式：每一个订阅的消费者都会收到消息
     */
     ...
     * <p>
     * <p>
     * 从哪里消费，【默认是从最后一个偏移量开始消费】
     * <p>
     * 一共有三种消费点：
     * 1、CONSUME_FROM_LAST_OFFSET
     *      客户端从上次停止的地方继续消费，也就是从最后一个偏移量开始消费，此处有两种情况：
    ...
    public DefaultMQPushConsumer(final String consumerGroup) {
        // 没有指定，默认使用的 AllocateMessageQueueAveragely （默认的消息负载均衡策略）
